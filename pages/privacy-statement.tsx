import Layout from 'components/Layout'
import HeroBanner from 'components/HeroBanner'
import Works from 'components/Works'

const PrivacyPage = () => {
  return (
    <>
      <style jsx>{`
        .privacy-page {
        }
      `}</style>
      <Layout htmlTitle="Privacy Statement | Mayfantasy">
        <div className="privacy-page max-w-1000px m-auto">
          {/* Hero Banner */}
          <HeroBanner
            title="Privacy Statement"
            includeContact
            src="/banner/privacy-banner.png"
            alt="Privacy Statement Banner"
          />
          <br />
          {/* Content */}
          <article>
            <p className="mb-3">
              mayfantasy.com is committed to protecting your privacy and
              developing technology that gives you the most powerful and safe
              online experience. This Statement of Privacy applies to the
              mayfantasy.com Web site and governs data collection and usage. By
              using the mayfantasy.com website, you consent to the data
              practises described in this statement.
            </p>

            <h2 className="mb-2">
              <strong>Collection of your Personal Information</strong>
            </h2>

            <p className="mb-3">
              mayfantasy.com collects personally identifiable information, such
              as your e-mail address, name, home or work address or telephone
              number. mayfantasy.com also collects anonymous demographic
              information, which is not unique to you, such as your postal code,
              age, gender, preferences, interests and favourites.
            </p>

            <p className="mb-3">
              There is also information about your computer hardware and
              software that is automatically collected by mayfantasy.com. This
              information can include: your IP address, browser type, domain
              names, access times and referring Web site addresses. This
              information is used by mayfantasy.com for the operation of the
              service, to maintain quality of the service, and to provide
              general statistics regarding use of the mayfantasy.com Web site.
            </p>

            <p className="mb-3">
              Please keep in mind that if you directly disclose personally
              identifiable information or personally sensitive data through
              mayfantasy.com public message boards, this information may be
              collected and used by others. Note: mayfantasy.com does not read
              any of your private online communications.
            </p>

            <p className="mb-3">
              mayfantasy.com encourages you to review the privacy statements of
              Web sites you choose to link to from mayfantasy.com so that you
              can understand how those Web sites collect, use and share your
              information. mayfantasy.com is not responsible for the privacy
              statements or other content on Web sites outside of the
              mayfantasy.com and mayfantasy.com family of Web sites.
            </p>

            <h2 className="mb-2">
              <strong>Use of your Personal Information</strong>
            </h2>

            <p className="mb-3">
              mayfantasy.com collects and uses your personal information to
              operate the mayfantasy.com Web site and deliver the services you
              have requested. mayfantasy.com also uses your personally
              identifiable information to inform you of other products or
              services available from mayfantasy.com and its affiliates.
              mayfantasy.com may also contact you via surveys to conduct
              research about your opinion of current services or of potential
              new services that may be offered.
            </p>

            <p className="mb-3">
              mayfantasy.com does not sell, rent or lease its customer lists to
              third parties. mayfantasy.com may, from time to time, contact you
              on behalf of external business partners about a particular
              offering that may be of interest to you. In those cases, your
              unique personally identifiable information (e-mail, name, address,
              telephone number) is not transferred to the third party. In
              addition, mayfantasy.com may share data with trusted partners to
              help us perform statistical analysis, send you email or postal
              mail, provide customer support, or arrange for deliveries. All
              such third parties are prohibited from using your personal
              information except to provide these services to mayfantasy.com,
              and they are required to maintain the confidentiality of your
              information.
            </p>

            <p className="mb-3">
              mayfantasy.com does not use or disclose sensitive personal
              information, such as race, religion, or political affiliations,
              without your explicit consent.
            </p>

            <p className="mb-3">
              mayfantasy.com keeps track of the Web sites and pages our
              customers visit within mayfantasy.com, in order to determine what
              mayfantasy.com services are the most popular. This data is used to
              deliver customized content and advertising within mayfantasy.com
              to customers whose behaviour indicates that they are interested in
              a particular subject area.
            </p>

            <p className="mb-3">
              mayfantasy.com Web sites will disclose your personal information,
              without notice, only if required to do so by law or in the good
              faith belief that such action is necessary to: (a) conform to the
              edicts of the law or comply with legal process served on
              mayfantasy.com or the site; (b) protect and defend the rights or
              property of mayfantasy.com; and, (c) act under exigent
              circumstances to protect the personal safety of users of
              mayfantasy.com, or the public.
            </p>

            <h2 className="mb-2">
              <strong>Use of Cookies</strong>
            </h2>

            <p className="mb-3">
              The mayfantasy.com Web site use "cookies" to help you personalize
              your online experience. A cookie is a text file that is placed on
              your hard disk by a Web page server. Cookies cannot be used to run
              programs or deliver viruses to your computer. Cookies are uniquely
              assigned to you, and can only be read by a web server in the
              domain that issued the cookie to you.
            </p>

            <p className="mb-3">
              One of the primary purposes of cookies is to provide a convenience
              feature to save you time. The purpose of a cookie is to tell the
              Web server that you have returned to a specific page. For example,
              if you personalize mayfantasy.com pages, or register with
              mayfantasy.com site or services, a cookie helps mayfantasy.com to
              recall your specific information on subsequent visits. This
              simplifies the process of recording your personal information,
              such as billing addresses, shipping addresses, and so on. When you
              return to the same mayfantasy.com Web site, the information you
              previously provided can be retrieved, so you can easily use the
              mayfantasy.com features that you customized.
            </p>

            <p className="mb-3">
              You have the ability to accept or decline cookies. Most Web
              browsers automatically accept cookies, but you can usually modify
              your browser setting to decline cookies if you prefer. If you
              choose to decline cookies, you may not be able to fully experience
              the interactive features of the mayfantasy.com services or Web
              sites you visit.
            </p>

            <h2 className="mb-2">
              <strong>Security of your Personal Information</strong>
            </h2>

            <p className="mb-3">
              mayfantasy.com secures your personal information from unauthorized
              access, use or disclosure. mayfantasy.com secures the personally
              identifiable information you provide on computer servers in a
              controlled, secure environment, protected from unauthorized
              access, use or disclosure. When personal information (such as a
              credit card number) is transmitted to other Web sites, it is
              protected through the use of encryption, such as the Secure Socket
              Layer (SSL) protocol.
            </p>

            <h2 className="mb-2">
              <strong>Changes to this Statement</strong>
            </h2>

            <p className="mb-3">
              mayfantasy.com will occasionally update this Statement of Privacy
              to reflect company and customer feedback. mayfantasy.com
              encourages you to periodically review this Statement to be
              informed of how mayfantasy.com is protecting your information.
            </p>

            <h2 className="mb-2">
              <strong>Contact Information</strong>
            </h2>

            <p className="mb-3">
              mayfantasy.com welcomes your comments regarding this Statement of
              Privacy. If you believe that mayfantasy.com has not adhered to
              this Statement, please contact mayfantasy.com at
              info@mayfantasy.com. We will use commercially reasonable efforts
              to promptly determine and remedy the problem.
            </p>
          </article>
          <br />
          <br />
          <br />
        </div>
      </Layout>
    </>
  )
}

export default PrivacyPage
